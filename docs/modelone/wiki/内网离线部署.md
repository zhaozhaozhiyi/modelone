...
if [ "$ARCH" = "x86_64" ]; then
  ...
  wget -O kubectl https://cube-studio.oss-cn-hangzhou.aliyuncs.com/install/kubectl-arm64 && mv kubectl-arm64 kubectl && chmod +x kubectl  && cp kubectl /usr/bin/ && mv kubectl /usr/local/bin/
fi
```
...
4、修改label studio镜像为内网镜像。
```bash
vi install/kubernetes/labelstudio/labelstudio.yaml
其中的 image的值更换为内网镜像
```

5、复制k8s的config文件，部署modelOne，部署方式通外网，参考：部署/单机部署
...
将联网机器上的aihub/modelscope/目录下的所有子目录 拷贝到内网服务器的/data/k8s/kubeflow/global/cube-studio/aihub/deep-learning/目录下面，

然后修改modelOne后台config.py配置文件中的
