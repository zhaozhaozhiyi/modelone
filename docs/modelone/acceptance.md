...
| 4. 标题、favicon、PWA、错误页 | 三个前端入口、清单和图标已通过真实 Nginx HTTP 检查；独立登录模板、错误页和品牌脚本均读取统一配置 | 正式配置、认证失败和服务异常页面实测 |
| 5. 不依赖原资源与镜像仓库 | 引用解析和迁移清单已实现；147 个镜像、939 个资源待筛选；门禁须当次验证企业目标摘要/校验和，仅下载不算完成 | 实际同步、镜像摘要与资源 SHA-256 比对、容器及浏览器网络检查 |
...
| 8. Docker/Kubernetes 部署 | Compose/Kustomize 渲染、前后端验证镜像构建、隔离后端启动及本地账号认证通过；认证、数据库和 Redis 私密注入检查通过；两个集群启动入口现在只接受已渲染的 modelOne 发布清单，并把所有源码 `-f/-k` 引用映射到镜像重写树，同时注入 `MODELONE_SERVICE_EXTERNAL_IP` | 企业镜像构建、基础设施生产配置、全量依赖部署、域名/TLS、企业认证与存储验证 |
...
本地品牌/发布回归二十九项、认证回归七项通过。2026-09-22 新增 Notebook 九项（八项通过、一项因未提供验证镜像而条件跳过）、节点部署五项、资源门禁八项和镜像包十项（九项通过、一项因未提供验证镜像而条件跳过）验证：Notebook 容器检查挂载当前启动脚本；节点测试替代 SSH/Docker，不连接真实节点；资源测试使用本地 HTTP 服务验证下载不足以发布、目标缺失、内容变化和断点校验；镜像包测试验证企业目标映射、归档校验、完整导入前检查、部署清单重写和批量重写 CLI。新增独立 modelOne 登录模板、`configure_modelone_remotes.py` 及回归测试，确保企业 `origin` 由参数显式设置、原仓库 `upstream` 禁止推送；镜像计划生成器可读取统一品牌配置，正式 Compose 产物改为企业镜像、命名数据卷和显式 kubeconfig 挂载。任务模板、三个前端 `public` 源目录、统一品牌配置、遗留主题模板和品牌静态资源均纳入扫描门禁；legacy `appbuilder` 静态资源移除了 source-map 引用，并删除了失效的 YOLOv8 旧安装钩子。
