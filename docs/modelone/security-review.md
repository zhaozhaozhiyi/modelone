...
2026-09-22 补充：Jupyter 初始化默认关闭 SSH，仍完成示例目录和 Spark 配置。SSH 可使用每个 Notebook 专用的公钥或密码文件；禁止空密码，凭据及系统密码更新失败阻止启动，平台初始化先于 IDE 完成。九项本地验证包含真实公钥解析和隔离容器中的启动失败传播；四种企业 Notebook 镜像尚未重建，实际 SSH 登录仍需集群验收。

节点批量工具先验证全部地址，按节点等待结果并汇总失败；私密 JSON 限制权限，通过 SSH 标准输入传输配置，终端不回显原始远端输出。四项本地测试覆盖失败传播、配置引用和输入拒绝，未连接实际节点。资源门禁使用本地 HTTP 夹具通过七项验证；旧报告不能代替当次 CDN/镜像目标校验。
