...
`--release` 要求企业仓库、资源 CDN、版权方、帮助、支持、用户协议和隐私地址配置，不等于完整发布验收。清单输出在 `dist/modelone`；未提供企业配置时可省略此参数生成开发预览。
